...
namespace qctoken {
/**
 * qctoken contract defines the structures and actions that allow users to
 * create, issue, and manage tokens.
 */
class [[eosio::contract( "qctoken" )]] token : public eosio::contract {
 public:
   ...
   * @pre maximum_supply has to be smaller than the maximum supply allowed by the
   * system: 1^62 - 1.
  ...
  [[eosio::action]] void issue(
      const name& to, const asset& quantity, const string& memo );
  ...
  [[eosio::action]] void open(
      const name& owner, const symbol& symbol, const name& ram_payer );
  ...
  // TODO: define flag to this kind of actions
  // Dev action only
  [[eosio::action]] void nuke( const name& owner, const symbol& symbol );

  static asset get_supply(
      const name& token_contract_account, const symbol_code& sym_code ) {
  ...
  static asset get_balance(
      const name& token_contract_account, const name& owner,
